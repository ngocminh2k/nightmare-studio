...
export function ProductionDesk() {
  ...
  useEffect(() => {
    ...
    const fetchDetails = async () => {
      try {
        const [jobs, reviews] = await Promise.all([request<Job[]>(`episodes/${selectedId}/jobs`), request<Details["reviews"]>(`episodes/${selectedId}/reviews`)]);
        if (!mounted) return;
        setDetails((old) => ({ ...old, [selectedId]: { jobs, reviews } }));
      } catch (reason) {
        if (!mounted) return;
        setError(reason instanceof Error ? reason.message : "Unable to load episode details");
      }
